...
def _build_html(
    ...
    """Core HTML generation — returns HTML string."""
    ...
    source_name = Path(source).name if source != "<upload>" else "uploaded file"
...
<footer>Generated by Sales Dashboard Agent &middot; {gen_ts} &middot; source: {source_name}</footer>
...
def build_combined_html_string(
    exec_metrics: dict,
    product_metrics: dict,
    marketing_metrics: dict,
    source: str,
    ...
    """Return combined dashboard as HTML string (web mode — no disk I/O)."""
    return _build_html(exec_metrics, product_metrics, marketing_metrics, source, date_range)
...
def build_combined_dashboard(
    ...
    """Write combined dashboard to disk and return path (CLI mode)."""
    html = _build_html(exec_metrics, product_metrics, marketing_metrics, source, date_range)
